...
private:
	...
	bool _insertar(Nodo<T>*& nodo, T e, function<int(T&, T&, int)> comparar, int columna) {
		...
		else if (comparar(nodo->elemento, e, columna) == 0)
			return false;
		else if (comparar(nodo->elemento, e, columna)>0) {
			_insertar(nodo->izq, e,comparar,columna);
		}
		else if (comparar(nodo->elemento, e, columna)<0) {
			_insertar(nodo->der, e,comparar, columna);
		}
		_balanceo(nodo);
		return true;
	}
	...
	void _inOrdenH(Nodo<T>* nodo) {
		...
		_inOrdenH(nodo->izq);
		vector<string> aux;
		aux.push_back(to_string(nodo->altura));
		procesar(aux);
		_inOrdenH(nodo->der);
	}

	bool _eliminar(Nodo<T>*& nodo, T e, function<int(T&, T&, int)> comparar, int columna) {
		if (nodo == nullptr) return false;
		else {
			int r = comparar(nodo->elemento, e, columna);
			if (r < 0) {
				return _eliminar(nodo->der, e, comparar,columna);
			}
			else if (r > 0) {
				return _eliminar(nodo->izq, e, comparar,columna);
			}
				...
				if (nodo->der == nullptr && nodo->izq == nullptr) {//caso 1
					...
					return _eliminar(nodo->der, aux->elemento, comparar,columna); //se envía a eliminar el elemento en el arbol por la derecha
				}
			}
		}
...
public:
	...
	bool Insertar(T e, function<int(T&, T&, int)>f, int columna) {
		return _insertar(raiz, e,f, columna);
	}
	void inOrden() {
		_inOrden(raiz);
	}
	...
	bool Eliminar(T e, function<int(T&, T&, int)>f, int columna) {
		bool val = _eliminar(raiz, e, f, columna);
		if (val) {
			_balanceo(raiz);
		}
		return val;
